...
abstract class ViewBindingItemProvider<T, R : ViewBinding> : BaseItemProvider<T>() {
    ...
    fun create(
        ...
        val h = QRecyclerViewBindHolder<R>(binding, binding.root)
        addChildClick(h)
        return h
    }

    open fun addChildClick(bindHolder: QRecyclerViewBindHolder<R>) {
...
abstract class QMultipleItemRvAdapter<T>() : QRecyclerAdapter<T>() {
    ...
    override fun onCreateDefViewHolder(parent: ViewGroup, viewType: Int): QRecyclerViewHolder {
        ...
        if (ViewBindingItemProvider::class.java.isAssignableFrom(p!!.javaClass)) {
            val binding = (p as ViewBindingItemProvider<*, *>).create(parent, context)
            return binding
        } else {
    ...
    private fun bindClick(
        ...
        val clickListener = this.getOnItemClickListener()
        val longClickListener = getOnItemLongClickListener()
